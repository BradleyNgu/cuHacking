...
class WasteSorterApp:
    ...
    def trigger_data_upload(self):
        """Trigger the data upload to the website after successful sorting"""
        try:
            # Log that we're starting an upload
            logger.info("Triggering automatic data upload to website")
            
            # Determine the path to the upload script
            script_dir = os.path.dirname(os.path.abspath(__file__))
            upload_script = os.path.join(script_dir, "upload_sort_data.py")
            
            # Run the upload script in a separate thread to avoid blocking the UI
            def run_upload():
                try:
                    # Execute the upload script with --force flag to ensure upload happens
                    result = subprocess.run(
                        [sys.executable, upload_script, "--force"],
                        capture_output=True,
                        text=True,
                    ...
                    if result.returncode == 0:
                        self.status_var.set("Data uploaded to website successfully")
    ...
    def sort_item_with_classification(self, classification, trigger_upload=True):
    ...
    def reset_after_sort(self):
        """Reset UI after sorting complete and trigger data upload"""
        self.is_sorting = False
        self.analyze_btn.state(['!disabled'])
        self.status_var.set("Ready for next item")
        
        # Trigger upload to website 
        self.trigger_data_upload()
